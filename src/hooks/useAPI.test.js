...
describe("Given the custom Hook useAPI", () => {
  ...
  describe("When loadLocalFishes is called with an array of local fishes", () => {
    test("Then it should return an array with fishes", async () => {
      const expectedLocalFishes = [
        {
          id: 1,
          "file-name": "bitterling",
          name: {
      ...
      const { result } = renderHook(() => useAPI(), { wrapper });
      const localFishes = await waitFor(() => result.current.loadLocalFishes());
      expect(localFishes).toEqual(expectedLocalFishes);
    });
  });
});
